...
local function start_server()
  if server_job_id then
    vim.notify("Markdown preview server is already running", vim.log.levels.INFO)
    return true
  end
  ...
  vim.notify("Markdown preview available at: " .. preview_url, vim.log.levels.INFO)
...
function M.start_preview()
  ...
  if server_job_id then
    ...
    if buf ~= current_buffer then
      ...
      local lines = {
        "Switch markdown preview to: " .. buf_name .. "?",
        "",
